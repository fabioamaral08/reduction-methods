...
__all__ = ['ParametricVAEModule', 'ParametricVAEModule_old']
...
class ParametricVAEModule(nn.Module):
    def __init__(self, n_input, latent_dim, num_params = 2, max_in = 1, min_in = 0, pred = False) -> None:
        ...
        self.encoder = nn.Sequential(
            nn.Linear(n_input + num_params,min_hidden*32), # 2^5
            nn.ReLU(),
            nn.Linear(min_hidden*32,min_hidden*16), # 2^4
            nn.ReLU(),
            nn.Flatten(), # Merge the channels
            nn.Linear(min_hidden*80,min_hidden*8), # 80 = 5 chanels x 16 dimensions
            nn.ReLU(),
            nn.Linear(min_hidden*8,min_hidden*4), # 2^3
            nn.ReLU(),
            nn.Linear(min_hidden*4,min_hidden), # 2^2
            nn.ReLU(),
        ) 
