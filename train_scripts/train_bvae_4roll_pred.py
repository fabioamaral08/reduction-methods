...
if __name__ == '__main__':
    ...
    autoencoder = Autoencoder.ParametricBVAEModule(n_input= train_dataset[0][0].shape[-1],latent_dim = latent_dim, num_params=2, max_in=upper_bound, min_in=lower_bound, small = True, pred=True).to(device)
    ...
    if loss_energy:
        def loss_fn(input:torch.Tensor, target:torch.Tensor, mu:torch.Tensor, log_var:torch.Tensor, param:torch.tensor, gamma = 0.0025):
                # reconst_loss = torch.nn.MSELoss()(input, target)
                reconst_loss = energy_loss(input, target, param)
                kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)
                const = latent_dim*0.5*torch.log(TWO_PI * gamma)
                reconst_weight = 0.5 * 1/gamma
                # kld_weight = 0.0025
    ...
    # Results directory
    pasta = f'/container/fabio/reduction-methods/ModelsTorch/BVAE_4RollOSC_Latent_{latent_dim}_energy_{loss_energy}'
    ...
    for e in range(num_epochs):
        ...
        for data,param in train_loader:
            ...
            reconst_loss, kld_loss, const, reconst_weight = loss_fn(data, reconst, mu, log_var, param, autoencoder.gamma)
            pred_loss = mse_loss(out_pred, forecast)
            loss = const + reconst_weight * reconst_loss + kld_loss + pred_loss
            loss.backward()
            optimizer.step()
            # save results (logging)
            cumm_loss += loss.item()
